...
export default function Disciplines({ onNavigate }: DisciplinesProps) {
  ...
  const showComingSoon = (discipline: Discipline) => {
    if (!user) return discipline.name !== 'Boxing';
    return !discipline.is_active && !isAdmin;
  };
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {disciplines.map((discipline) => {
            const comingSoon = showComingSoon(discipline);
              ...
              >
                ...
                {user ? (
                  <button
                    ...
                    disabled={comingSoon}
                    className={`mt-1 focus:outline-none group w-full ${comingSoon ? 'cursor-not-allowed opacity-60' : ''}`}
                  >
                  ...
                  <div className={`mt-1 w-full ${comingSoon ? 'opacity-60' : ''}`}>
                    <div className="w-full h-[205px] sm:h-[256px] flex items-center justify-center">
                      <img
                        src={buttonImages[discipline.name] || buttonImages['Boxing']}
                        alt={`${discipline.name}`}
