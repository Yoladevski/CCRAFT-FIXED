...
export default function Home({ onNavigate }: HomeProps) {
  const { user } = useAuth();
  const [showCombatCraftModal, setShowCombatCraftModal] = useState(false);
  ...
  return (
    <div>
      ...
      <section className="py-16 sm:py-24 px-4 bg-[#0E0E0E] relative">
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-12 mt-[10px]">
            <button
              onClick={() => setShowCombatCraftModal(true)}
      ...
      {showCombatCraftModal && (
        <div
          className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4"
          onClick={() => setShowCombatCraftModal(false)}
        >
          <div
            className="bg-[#1A1A1A] rounded-lg border border-[#B11226] max-w-2xl w-full max-h-[90vh] overflow-y-auto"
            style={{
              boxShadow: '0 0 30px rgba(177, 18, 38, 0.4), 0 0 60px rgba(177, 18, 38, 0.2)'
            }}
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 bg-[#1A1A1A] p-4 sm:p-6 border-b border-[#B11226]/30 flex items-center justify-between">
              <h3 style={{
                fontFamily: 'Orbitron, sans-serif',
                fontWeight: 900,
                fontSize: '19.2px',
            ...
            <div className="p-4 sm:p-6 space-y-4" style={{ fontFamily: 'Orbitron, sans-serif', fontSize: '11.2px', fontWeight: 500, color: '#A0A0A0', lineHeight: '1.8' }}>
